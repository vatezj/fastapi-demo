...
async def app_user_login(
    ...
    """APP用户登录"""
    return await AppUserService.app_login(login_data, request)


@app_user_router.post("/register")
async def app_user_register(register_data: AppRegisterModel):
    """APP用户注册"""
    return await AppUserService.app_register(register_data)


@app_user_router.post("/send-sms")
async def send_sms_code(sms_data: AppSmsCodeModel):
    """发送短信验证码"""
    return await AppUserService.send_sms_code(sms_data)
...
@app_user_router.delete("/login-log/delete")
...
@app_user_router.delete("/login-log/clean")
