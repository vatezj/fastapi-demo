    ...
    async def export_operation_log_list_services(cls, request: Request, operation_log_list: List):
        ...
        if redis:
            operation_type_list = await DictDataService.query_dict_data_list_from_cache_services(
                redis, dict_type='sys_oper_type'
            )
        else:
            logger.warning('Redis不可用，使用默认操作类型')
            operation_type_list = []
...
class LoginLogService:
    ...
    async def unlock_user_services(cls, request: Request, unlock_user: UnlockUser):
        ...
        redis = await RedisUtil.get_redis_pool()
        if not redis:
            logger.warning('Redis不可用，跳过用户解锁')
            return CrudResponseModel(is_success=True, message='解锁成功（Redis不可用）')
        
        try:
            locked_user = await redis.get(f'account_lock:{unlock_user.user_name}')
            if locked_user:
                await redis.delete(f'account_lock:{unlock_user.user_name}')
                return CrudResponseModel(is_success=True, message='解锁成功')
            else:
                raise ServiceException(message='该用户未锁定')
        except Exception as e:
            logger.warning(f'Redis操作失败: {e}')
            return CrudResponseModel(is_success=True, message='解锁成功（Redis操作失败）')
