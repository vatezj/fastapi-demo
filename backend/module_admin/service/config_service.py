...
class ConfigService:
            ...
            try:
                ...
                if redis:
                    try:
                        ...
                        logger.warning(f'Redis缓存更新失败: {e}')
                else:
                    logger.warning('Redis不可用，跳过缓存更新')
    ...
    async def delete_config_services(cls, request: Request, query_db: AsyncSession, page_object: DeleteConfigModel):
        ...
        if page_object.config_ids:
            ...
            try:
                ...
                # 获取Redis连接并删除缓存
                redis = await RedisUtil.get_redis_pool()
                if redis and delete_config_key_list:
                    try:
                        await redis.delete(*delete_config_key_list)
                    except Exception as e:
                        logger.warning(f'Redis缓存删除失败: {e}')
                elif not redis:
                    logger.warning('Redis不可用，跳过缓存删除')
    ...
    async def refresh_sys_config_services(cls, request: Request, query_db: AsyncSession):
        ...
        redis = await RedisUtil.get_redis_pool()
        if redis:
            await cls.init_cache_sys_config_services(query_db, redis)
        else:
            logger.warning('Redis不可用，跳过缓存刷新')
